...
@Controller
public class AddmyBBSItemController {
	...
	public ModelAndView onSubmit(HttpSession session, HttpServletRequest request, BBSItem item)
	{
		...
		item.setPostTarget(request.getParameter("writer"));
		item.setContent(request.getParameter("content"));
		
		BBSController.getInstance().getManager().register(request.getParameter("writer"), item);
		
		//System.out.println("AddmyBBSItemController 29 "+item);
